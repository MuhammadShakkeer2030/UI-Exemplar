...
export function Hero() {
  return (
    ...
    //     </div>
    // </div>
    <div className="w-full md:w-[80%] mx-auto bg-white p-4">
  ...
  <div className="flex justify-center md:justify-end mb-4">
    ...
    <div className="flex space-x-2">
      <button className="bg-primary px-3 md:px-4 py-2 rounded-full font-semibold text-xs md:text-base">Monthly</button>
      <button className="px-3 md:px-4 py-2 rounded-full relative font-semibold text-xs md:text-base">Annually<span className="text-green-500 text-xs md:text-sm absolute top-0 right-2">-10%</span></button>
    </div>
  </div>

  <div className="flex flex-col lg:flex-row justify-center lg:justify-evenly items-center gap-3 mt-5">
    {planItems?.map((plan, index) => (
      <PlanCard key={index} plan={plan} quickPlan={undefined} index={index} customstyle={'w-full md:w-2/3 lg:w-[300px]'} />
    ))}
  </div>
  ...
  <div className="flex flex-col lg:flex-row justify-center lg:justify-between items-center gap-3 mt-4">
    {quickPlans?.map((plan, index) => (
      <PlanCard key={index} quickPlan={plan} plan={undefined} index={index} customstyle="w-full md:w-4/5 lg:w-[45%] xl:w-[50%] mx-2 md:mx-0" />
    ))}
  </div>
</div>
