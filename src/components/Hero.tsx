...
export function Hero() {
  ...
  <div className="w-[350px] md:w-auto py-5 mx-auto">
  ...
  <div className="flex justify-end">
    {/* Slide button */}
    <div className="border-4 rounded-full">
      <button className="bg-primary px-4 py-2 rounded-full font-semibold text-xs md:text-base">Monthly</button>
      <button className="px-4 py-2 rounded-full relative font-semibold text-xs md:text-base">Annually<span className="text-green-500 text-xs md:text-sm absolute top-0 right-2">-10%</span></button>
    </div>
  </div>

  <div className="flex flex-col lg:flex-row justify-evenly items-center gap-3 mt-5">
    {planItems?.map((plan, index) => (
      <PlanCard key={index} plan={plan} quickPlan={undefined} index={index} customstyle={'w-[350px] md:w-2/3  lg:w-1/4'} />
    ))}
  </div>

  <div className="flex flex-col lg:flex-row items-center gap-3 mt-4">
    {quickPlans?.map((plan, index) => (
      <PlanCard key={index} quickPlan={plan} plan={undefined} index={index} customstyle="w-auto md:w-4/5 lg:1/2 mx-5 md:mx-0" />
    ))}
  </div>
</div>
